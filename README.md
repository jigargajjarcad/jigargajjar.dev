...
| **10 minutes** | [`docs/DECISIONS.md`](docs/DECISIONS.md) — 19 ADRs, each with alternatives considered and why they were rejected |
...
└── wireframes/            9 documents — one per route, mobile-first
src/                       Application: routes, primitives, layout, content layer, tokens
content/case-studies/      MDX case studies, validated against a Zod schema at build time
tests/                     44 unit tests, 76 Playwright quality tests
scripts/                   Token generation and the budget, token, and link gates
```
...
| 5 — Implementation | Application builds against the specification; native motion pending (ADR-018) |
| 6 — Content | In progress — first case study drafted |
